...
export class CalendarsService {
  ...
  getMicrosoftAuthUrl(userId: string): string {
    const state = this.encodeState({ userId, provider: 'MICROSOFT' });
  ...
  async handleMicrosoftCallback(code: string, state: string) {
    const { userId } = this.decodeState(state);
    const tokens = await this.microsoft.exchangeCode(code);
  ...
  async getFreeBusyForUser(
    ...
    for (const conn of connections) {
      const intervals = await this.fetchFreeBusy(
        conn.provider,
        this.crypto.decrypt(conn.accessToken),
        conn.calendarId,
        timeMin,
        timeMax,
      );
  ...
  async getFreeBusyForParticipant(
    participantId: string,
    timeMin: Date,
    timeMax: Date,
  ): Promise<BusyInterval[]> {
    const access = await this.prisma.anonymousCalendarAccess.findUnique({
      where: { participantId },
    });
    if (!access) return [];
    ...
    return this.fetchFreeBusy(
      access.provider,
      this.crypto.decrypt(access.accessToken),
      access.calendarId,
      timeMin,
      timeMax,
    );
  ...
  // ── Anonymous access save (called from InviteService) ────────────────────

  async saveAnonymousAccess(
    participantId: string,
    provider: ProviderEnum,
    tokens: any,
  ) {
